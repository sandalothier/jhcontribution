...
import com.test.contribution.config.KafkaProperties;
...
class ContributionKafkaResourceIT {
    ...
    void setup() {
        ...
        ContributionKafkaResource kafkaResource = new ContributionKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/contribution-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/contribution-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
